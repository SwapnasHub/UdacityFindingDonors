...
# **Answer: ** AdaBoost (http://mccormickml.com/2013/12/13/adaboost-tutorial/)
...
# **Answer: ** Gradient Boosting starts by picking a simple, but ill informed solution to the problem. It will then try to improve this solution in steps, until its built a model. For instance in this case the first solution may be that all people have higher than 50K wage. It will then check how well it has done and compute a residual, which is the difference between the true value and the expected one.
# 
...
# **Answer: **Optimised scores are better than both the benchmark and unoptimised gradient boosting model, for both accuracy and F-score. The parameter optimised is max_depth. This controls the maximum depth of a tree. The default is 3, and the optimised value is 6. We have 103 features after hot-coding. The more splits a tree can make, the more complex interactions it can account for across these features.
...
# **Answer:** 1 & 2: education-num & education. Better education would more often than not allow more opportunities in job positions with higher earnings. I think the education-num variable may be more useful, especially if there are some factors of the education variable that has low quantities.
# 
...
# **Answer:**The model results after running with the top 5 variables is very good. We only lose 1.3% accuracy (0.87 to 0.8588) & 3.3% F-score value (0.7498 to 0.7247) after moving to the top 5 variables.
